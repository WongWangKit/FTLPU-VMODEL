...
module lpu_vxm_fp32_arithmetic_tb;
  timeunit 1ns;
  timeprecision 1ps;
  ...
  initial begin
    $readmemh("sim/vectors/vxm_fp32_arithmetic_init.hex", cmodel_init);
    $readmemh("sim/vectors/vxm_fp32_arithmetic_golden.hex", cmodel_golden);
    $readmemh("sim/vectors/vxm_fp32_arithmetic_schedule.hex", cmodel_schedule);
    ...
    if (queue_fault != '0 || mem_fault != '0 || sxm_fault != '0 ||
      ...
      $fatal(1,
        "VXM arithmetic fault queue=%h mem=%h sxm=%b vxm=%b conflict=%b",
