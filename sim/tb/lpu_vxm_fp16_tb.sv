...
module lpu_vxm_fp16_tb;
  ...
  logic [63:0] cycle;
  logic [63:0] cmodel_init [0:31];
  logic [63:0] cmodel_golden [0:31];
  logic [479:0] cmodel_schedule [0:25];

  lpu_top #(
    .ICU_QUEUE_DEPTH(4),
    .MEM_DEPTH_ROWS(16),
    .ACTIVE_MEM_COLUMNS(8)
  ) dut (
    .clk_i(clk),
    .rst_ni(rst_n),
    .run_i(run),
  ...
  initial begin
    $readmemh("sim/vectors/vxm_fp16_relu_init.hex", cmodel_init);
    $readmemh("sim/vectors/vxm_fp16_relu_golden.hex", cmodel_golden);
    $readmemh("sim/vectors/vxm_fp16_relu_schedule.hex", cmodel_schedule);
    ...
    if (queue_fault != '0 || mem_fault != '0 || sxm_fault != '0 ||
      ...
      $fatal(1,
        "VXM FP16 regression fault queue=%h mem=%h sxm=%b vxm=%b conflict=%b",
    ...
    for (integer column = 0; column < 8; column++) begin
      ...
      for (integer tile = 0; tile < 4; tile++) begin
        ...
        if (host_read_data !== cmodel_golden[column*4+tile])
          $fatal(1,
            "VXM float column %0d tile %0d mismatch got=%h expected=%h",
            column, tile, host_read_data,
            cmodel_golden[column*4+tile]);
      end
    end

    $display("FTLPU-VMODEL C-model VXM FP16/BF16/FP32 chain passed in %0d cycles",
             cycle);
    $finish;
  end
endmodule
