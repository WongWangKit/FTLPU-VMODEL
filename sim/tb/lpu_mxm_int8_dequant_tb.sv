...
module lpu_mxm_int8_dequant_tb;
  timeunit 1ns;
  timeprecision 1ps;

  localparam integer INIT_WORDS = 136;
  localparam integer GOLDEN_WORDS = 16;
  localparam integer SCHEDULE_RECORDS = 48;
  ...
  initial begin
    $readmemh("sim/vectors/mxm_int8_dequant_init.hex", cmodel_init);
    $readmemh("sim/vectors/mxm_int8_dequant_golden.hex", cmodel_golden);
    $readmemh("sim/vectors/mxm_int8_dequant_schedule.hex", cmodel_schedule);
      ...
      for (integer column = 0; column < 8; column++)
        for (integer tile = 0; tile < 4; tile++)
          host_write_tile(
            ...
            cmodel_init[(block*8+column)*4+tile]);

    for (integer byte_index = 0; byte_index < 2; byte_index++)
      for (integer tile = 0; tile < 4; tile++)
        host_write_tile(
          (16+byte_index), tile[1:0], 16'd4,
          cmodel_init[128+byte_index*4+tile]);
    ...
    if (queue_fault != '0 || mem_fault != '0 || mxm_fault != '0 ||
      ...
      $fatal(1,
        "MXM INT8 fault queue=%h mem=%h mxm=%b sxm=%b vxm=%b conflict=%b",
    ...
    for (integer byte_index = 0; byte_index < 4; byte_index++) begin
      ...
      for (integer tile = 0; tile < 4; tile++) begin
        ...
        if (host_read_data !== cmodel_golden[byte_index*4+tile])
          $fatal(1,
            "MXM INT8 mismatch byte=%0d tile=%0d got=%h expected=%h",
            byte_index, tile, host_read_data,
            cmodel_golden[byte_index*4+tile]);
      end
    end

    $display("FTLPU-VMODEL C-model MXM INT8 dequant passed in %0d cycles",
             cycle);
    $finish;
  end
endmodule
